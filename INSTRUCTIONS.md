...
| Приоритет | Задача | Почему важно | Статус |
|---|---|---|---|
| ВЫСОКИЙ | Карта маршрута (Leaflet/MapLibre) | "карта с положением поезда" — прямое требование | НЕ СДЕЛАНО |
| ВЫСОКИЙ | KZ8A-адаптивные панели на дашборде | Дашборд хардкодит TE33A поля, KZ8A ломается | НЕ СДЕЛАНО |
| ВЫСОКИЙ | WebSocket reconnect + backoff на фронтенде | Оценивается в критерии "Real-time 35%" | НЕ СДЕЛАНО |
| ВЫСОКИЙ | UI переключатель сценариев | Демо без переключения = потерянные баллы | НЕ СДЕЛАНО |
| ВЫСОКИЙ | Показать recommendation в алертах | "рекомендации действий" — прямое требование | НЕ СДЕЛАНО |
| СРЕДНИЙ | Доп. панели: масло, выхлоп, турбо, подшипники, вентиляторы | Полнота мониторинга узлов | НЕ СДЕЛАНО |
| СРЕДНИЙ | PDF-экспорт отчёта | В документе явно написано "PDF/CSV" | НЕ СДЕЛАНО |
| СРЕДНИЙ | Индикатор скорость vs лимит | speed_limit данные есть, не используются | НЕ СДЕЛАНО |
| СРЕДНИЙ | `.gitignore` | Инженерная культура (10%) | НЕ СДЕЛАНО |
| СРЕДНИЙ | Архитектурная диаграмма | Нужна для презентации 10-12 слайдов | НЕ СДЕЛАНО |
| НИЗКИЙ | Светлая/тёмная тема | Упомянуто в требованиях | НЕ СДЕЛАНО |
| НИЗКИЙ | Метрики сервиса (счётчики WS-соединений) | Улучшает оценку Engineering Culture | НЕ СДЕЛАНО |
...
## 8. Аудит дашборда — что показывается vs что требуется

> Обновлено: 2026-04-04

### Что сейчас отображается на дашборде (`web-app/app/dashboard/page.tsx`)

| Секция | Данные |
|---|---|
| Sidebar: Индекс здоровья | `health.index` (0-100), `health.grade` (A-E), group_scores бары |
| Sidebar: Алерты | Счётчики critical/warning/info + список (label, message) |
| Top Cards (5 шт.) | Скорость, Топливо %, Темп. ОЖ, Давл. ТМ, Обороты |
| Тренд-график | Скорость, Темп ТЭД, Топливо, Ток ТЭД (переключаемые, 1/5/15 мин) |
| Температуры ТЭД | По осям с прогресс-барами |
| Навигация + Факторы | lat, lon, одометр, сигнал светофора + top-5 факторов индекса |

### ЧТО НЕ ХВАТАЕТ (gap analysis)

| Приоритет | Что отсутствует | Требование из кейса | Где данные |
|---|---|---|---|
| **ВЫСОКИЙ** | Интерактивная карта маршрута (Leaflet/MapLibre) | "карта маршрута с текущим положением и ограничениями" | `navigation.latitude/longitude/speed_limit` |
| **ВЫСОКИЙ** | Панели для KZ8A (напряжение КС, шина DC, тяговая мощность) | Дашборд хардкодит TE33A поля — KZ8A показывает пустоту | `resources.catenary_voltage/dc_bus_voltage/traction_power` |
| **ВЫСОКИЙ** | WebSocket reconnect + exponential backoff | "отказоустойчивость: reconnect, backoff" (35% оценки!) | Frontend logic |
| **ВЫСОКИЙ** | Переключатель сценариев в UI | Backend поддерживает 8 сценариев, но нет UI | WS message `{"scenario": "..."}` |
| **СРЕДНИЙ** | Рекомендации в алертах | `recommendation` поле есть но не рендерится | `alert.recommendation` |
| **СРЕДНИЙ** | Доп. панели ресурсов ТЭ33А: масло (темп/давл), выхлоп, расход топлива, турбо | "Давления/Температуры" панель | `resources.*` |
| **СРЕДНИЙ** | Температуры подшипников | Данные есть, не визуализируются | `nodes.axle_bearing_temps` |
| **СРЕДНИЙ** | Статусы вентиляторов | Данные есть, не визуализируются | `nodes.cooling_fan_statuses` |
| **СРЕДНИЙ** | Индикатор превышения скорости (текущая vs лимит) | `speed_limit` поле не используется | `navigation.speed_limit` |
| **СРЕДНИЙ** | Тяговое усилие, позиция контроллера | Не отображаются | `traction.tractive_effort/controller_position` |
| **НИЗКИЙ** | PDF-экспорт | Кейс требует "PDF/CSV" | Бэкенд: только CSV/JSON |
| **НИЗКИЙ** | Светлая/тёмная тема (переключатель) | Только тёмная тема | Frontend CSS |
| **НИЗКИЙ** | Давление тормозных цилиндров + тормозное усилие | Только давление ТМ показано | `traction.brake_cylinder_pressure/brake_force` |

### TypeScript проблема: `TelemetryFrame.resources`

В `web-app/lib/api.ts` тип `resources` объединён в одну плоскую структуру с optional полями.
Но дашборд (`page.tsx`) обращается к `frame.resources.fuel_level`, `frame.resources.coolant_temperature`,
`frame.resources.engine_rpm` **без проверки loco_type**. При подключении KZ8A эти поля будут `undefined`,
и карточки покажут "—". Нужно условное отображение в зависимости от `frame.loco_type`.

---

## 9. Критические вещи которые нельзя забыть до сдачи
